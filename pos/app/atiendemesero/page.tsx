...
export default function AtiendemeseroPage() {
  ...
  useEffect(() => {
    ...
    // Obtener mesero actual desde API o localStorage como fallback
    const fetchCurrentUser = async () => {
      try {
        ...
        if (response.ok) {
          ...
          // Si el API falla, usar localStorage
          const storedUser = localStorage.getItem('pos_user')
          if (storedUser) {
            const user = JSON.parse(storedUser)
            setMeseroId(user.id)
            console.log('Mesero cargado desde localStorage:', user.nombre)
          }
        }
      } catch (error) {
        ...
        if (storedUser) {
          const user = JSON.parse(storedUser)
          setMeseroId(user.id)
          console.log('Mesero cargado desde localStorage (fallback):', user.nombre)
        }
      }
    }
  ...
  const sendOrderAuthorized = async () => {
    ...
    try {
      ...
      if (continuePedidoId) {
        ...
        // Verificar meseroId - si es 4 (default), intentar obtener del token
        let finalMeseroId = meseroId;
        if (finalMeseroId === 4) {
          try {
            const response = await fetch('/pos/api/auth/me');
            if (response.ok) {
              ...
              const user = result.data || result;
              if (user && user.id) {
                finalMeseroId = user.id;
                console.log('Mesero ID obtenido del token:', user.nombre, '(ID:', finalMeseroId + ')');
              }
            }
          } catch (error) {
        ...
        const orderData = {
          mesero_id: finalMeseroId, // Usar mesero_id verificado
        ...
        if (response.ok) {
          ...
          if (cuentaId) {
            localStorage.setItem('pedido_agregado_exito', JSON.stringify({
              numero_pedido: responseData.pedido?.numero_pedido || 'Nuevo pedido',
              total: responseData.pedido?.total || total,
              timestamp: Date.now()
            }));
