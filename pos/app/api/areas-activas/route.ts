  ...
  try {
    ...
    const query = `
      SELECT
        ...
        CASE WHEN c.mesa_numero LIKE 'PL%' THEN 1 ELSE 0 END as es_para_llevar
      FROM cuentas c
      LEFT JOIN mesas m ON c.mesa_numero = m.numero
      WHERE c.estado IN (${stateFilter})
      ORDER BY CASE WHEN c.mesa_numero LIKE 'PL%' THEN 1 ELSE 0 END DESC, c.fecha_apertura DESC
    `;
