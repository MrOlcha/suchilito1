    ...
    for (const categoria of categorias as any[]) {
      ...
      const items = db.prepare(
        `SELECT id, nombre, descripcion, precio, imagen_url, imagen_local,
                nuevo, vegetariano, picante, favorito, destacado, promomiercoles
         FROM menu_items
         WHERE categoria_id = ? AND activo = 1 AND precio > 0
         ORDER BY nombre`
      ).all(categoria.id);
      ...
      const menuItems: MenuItem[] = (items as any[]).map((item: any) => {
        ...
        // Convertir rutas de imagen a API endpoint
        if (imageUrl) {
          // Si es ruta de API, agregar basePath
          if (imageUrl.startsWith('/api/menu-images/')) {
            imageUrl = `/pos${imageUrl}`;
          } 
          // Si es ruta de menu-images, convertir a API
          else if (imageUrl.startsWith('/menu-images')) {
            const filename = imageUrl.split('/').pop();
            imageUrl = `/pos/api/menu-images/${filename}`;
          }
          // Si no tiene basePath, agregar
          else if (!imageUrl.startsWith('/pos')) {
            imageUrl = `/pos${imageUrl}`;
          }
        }
        return {
...
export async function getMenuItemCount(): Promise<number> {
  ...
  try {
    const result = db.prepare('SELECT COUNT(*) as count FROM menu_items WHERE activo = 1').get() as any;
