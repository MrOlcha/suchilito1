  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-gray-50 to-white p-2 sm:p-3 md:p-4 lg:p-6">
      ...
      <div className="w-full mb-3 sm:mb-4 md:mb-6">
        <div className="bg-white rounded-lg sm:rounded-xl md:rounded-2xl shadow-md p-3 sm:p-4 md:p-6 border border-gray-200">
          <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-2 sm:gap-4">
            <div>
              <h1 className="text-xl sm:text-2xl md:text-4xl font-bold text-gray-900 mb-0.5 sm:mb-2">🍽️ Mesas</h1>
              <p className="text-blue-600 text-xs sm:text-sm md:text-lg font-medium">Elige una mesa</p>
            </div>
            <button
              onClick={() => router.push('/atiendemesero')}
              className="w-full sm:w-auto px-3 sm:px-6 md:px-8 py-2 sm:py-3 md:py-4 bg-gradient-to-r from-gray-200 to-gray-300 text-gray-900 rounded-lg sm:rounded-xl md:rounded-xl hover:from-gray-300 hover:to-gray-400 transition-all transform hover:scale-105 font-bold text-xs sm:text-sm md:text-lg shadow-md"
            >
              ← Volver
            </button>
          </div>
      ...
      <div className="w-full mb-3 sm:mb-4 md:mb-8">
        <div className="bg-white rounded-lg sm:rounded-xl md:rounded-2xl shadow-md p-3 sm:p-4 md:p-6 border border-gray-200">
          <h3 className="text-xs sm:text-sm md:text-lg font-bold text-gray-900 mb-2 sm:mb-4">Estado:</h3>
          <div className="flex flex-wrap gap-2 sm:gap-3 md:gap-6">
            <div className="flex items-center">
              <div className="w-3 h-3 sm:w-4 sm:h-4 md:w-5 md:h-5 bg-gradient-to-br from-green-500 to-emerald-600 rounded mr-2 sm:mr-3 shadow-md"></div>
              <span className="text-gray-700 font-medium text-xs sm:text-sm md:text-base">Libre</span>
            </div>
            <div className="flex items-center">
              <div className="w-3 h-3 sm:w-4 sm:h-4 md:w-5 md:h-5 bg-gradient-to-br from-orange-500 to-red-500 rounded mr-2 sm:mr-3 shadow-md"></div>
              <span className="text-gray-700 font-medium text-xs sm:text-sm md:text-base">Ocupada</span>
            </div>
            <div className="flex items-center">
              <div className="w-3 h-3 sm:w-4 sm:h-4 md:w-5 md:h-5 bg-gradient-to-br from-red-600 to-red-800 rounded mr-2 sm:mr-3 shadow-md"></div>
              <span className="text-gray-700 font-medium text-xs sm:text-sm md:text-base">Reservada</span>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="max-w-7xl mx-auto">
        {mesas.length === 0 ? (
          <div className="bg-white rounded-2xl shadow-md p-16 text-center border border-gray-200">
            ...
            <p className="text-gray-900 text-2xl font-bold mb-3">No hay mesas disponibles</p>
            <p className="text-gray-600 text-lg">Contacta al administrador para configurar las mesas</p>
          </div>
        ) : (
          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-2 sm:gap-3 md:gap-4 max-w-7xl mx-auto">
            {mesas.map((mesa) => (
              <button
                ...
                className={`${getEstadoColor(mesa.estado)} text-white rounded-lg md:rounded-xl shadow-md md:shadow-lg transition-all transform hover:scale-105 md:hover:scale-110 hover:-translate-y-1 md:hover:-translate-y-2 p-2 sm:p-3 md:p-4 lg:p-6 flex flex-col items-center justify-center relative overflow-hidden group border-2 disabled:cursor-not-allowed`}
              >
