...
interface MenuParaLlevarProps {
  cuentaInfo?: CuentaInfo | null;
  loadingCuenta?: boolean;
}

export default function MenuParaLlevar({ cuentaInfo, loadingCuenta = false }: MenuParaLlevarProps) {
  const router = useRouter();
  const { cart, agregarAlCarrito, eliminarDelCarrito, actualizarCantidad, calcularTotal, calcularSubtotal, calcularDescuentoTotal, descuentosAplicados, mesaNumero, cuentaId, limpiarTodo, setPromociones: setPromocionesContext } = usePedidoContext();
  ...
  const handleFinalizarPedido = async () => {
    ...
    try {
      ...
      // Obtener mesero_id desde AuthService (localStorage)
      let meseroId = AuthService.obtenerMeseroId();
      console.log('Mesero ID inicial desde AuthService:', meseroId);
      
      // Si tiene ID 4 (default), intentar obtener desde API usando el token
      if (meseroId === 4) {
        try {
          ...
          if (response.ok) {
            ...
            if (user && user.id) {
              meseroId = user.id;
              console.log('Mesero ID obtenido desde API:', user.nombre, '(ID:', meseroId + ')');
            }
          }
        } catch (error) {
      ...
      // Crear pedido - el backend generará automáticamente el número PLXXXX
      const response = await fetch('/pos/api/pedidos', {
        ...
        body: JSON.stringify({
          mesa_numero: null, // El backend lo determinará automáticamente
          mesero_id: meseroId,
          comensales: 1,
          es_para_llevar: 1,
          cuenta_id: cuentaId,
      ...
      // Si se agregó a una cuenta existente, redirigir a áreas activas con notificación
      if (cuentaId) {
        localStorage.setItem('pedido_agregado_exito', JSON.stringify({
          numero_pedido: numeroPedido,
          total: parseFloat(total),
          timestamp: Date.now()
        }));
        router.push('/areas-activas?success=pedido_agregado');
        return;
      }
        ...
        <div className="flex items-center justify-between gap-2">
          <div className="flex flex-col">
            <h1 className="text-base sm:text-lg font-bold text-white truncate">
              {cuentaInfo ? (
                cuentaInfo.es_para_llevar === 1 ? 'Agregar a Pedido Para Llevar' : `Agregar a Mesa ${cuentaInfo.mesa_numero || 'N/A'}`
              ) : (
                'Pedido Para Llevar'
              )}
            </h1>
            {cuentaInfo && (
              <p className="text-xs sm:text-sm text-gray-300">
                Cuenta: {cuentaInfo.numero_cuenta}
                {cuentaInfo.es_para_llevar === 1 ? ' • Para Llevar' : ' • Mesa'}
              </p>
            )}
            {loadingCuenta && (
            ...
            >
              <div className="bg-gray-800 rounded-3xl w-full max-w-md max-h-[90vh] flex flex-col overflow-hidden shadow-2xl pointer-events-auto">
                ...
                <div className="px-3 sm:px-4 py-2 sm:py-3 border-b border-gray-700 flex-shrink-0 bg-gray-750">
                  ...
                  <p className="text-xs sm:text-sm text-orange-400 font-medium">Para Llevar • {obtenerNombreDia()}</p>
